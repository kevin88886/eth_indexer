...
type (
	...
	IERC20Freeze struct {
		...
		Payment  protocol.EntrustMessagePayment `json:"payment"`
	}
...
func (parser *IERC20Parser) Parse(tx *domain.Transaction) (protocol.IERCTransaction, error) {
	...
	fmt.Println("ierc20_parser")
	fmt.Println(string(data))
	signVersion := 3
	if strings.Contains(string(data), "\"version\":\"4.0\"") {
		signVersion = 4
	}
	var ierc20 IERC20
	...
	case protocol.OpFreezeSell:
		if signVersion == 3 {
			...
			return parser.parseFreezeSellV4(base, &ierc20)
		}
	...
	case protocol.OpProxyTransfer:
		if signVersion == 3 {
			...
			return parser.parseProxyTransferV4(base, &ierc20)
		}
...
func (parser *IERC20Parser) parseNonce(tick string, value interface{}) (decimal.Decimal, error) {
	if tick == parser.ethi {
		...
		// 非ethi的币种必须是: 正整数字符串, 且前缀未能为0. 例如: "01000"
		// 老版本可能存在 nonce 为数值类型的的问题
...
func (parser *IERC20Parser) parseFreezeSellV4(base protocol.IERCTransactionBase, ierc20 *IERC20) (*protocol.FreezeSellCommandV4, error) {
	var tick = strings.TrimSpace(ierc20.Tick)

	var records = make([]protocol.FreezeRecordV4, 0, len(ierc20.Freeze))
	for _, freeze := range ierc20.Freeze {
		...
		records = append(records, protocol.FreezeRecordV4{
			Protocol:   base.Protocol,
			Operate:    base.Operate,
			Tick:       tick,
			Platform:   strings.ToLower(strings.TrimSpace(freeze.Platform)),
			...
			Amount:     amount,
			Value:      ethValue,
			Expire:     freeze.Expire,
			Version:    "4.0",
			Payment:    freeze.Payment,
		})
	}

	return &protocol.FreezeSellCommandV4{IERCTransactionBase: base, Records: records}, nil
}
...
func (parser *IERC20Parser) parseProxyTransferV4(base protocol.IERCTransactionBase, ierc20 *IERC20) (*protocol.ProxyTransferCommandV4, error) {
	var records = make([]protocol.ProxyTransferRecordV4, 0, len(ierc20.Proxy))
	for _, proxy := range ierc20.Proxy {
		...
		records = append(records, protocol.ProxyTransferRecordV4{
			Protocol:    base.Protocol,
			Operate:     base.Operate,
			Tick:        tick,
			From:        strings.ToLower(strings.TrimSpace(proxy.From)),
	...
	return &protocol.ProxyTransferCommandV4{IERCTransactionBase: base, Records: records}, nil
}
...
func (parser *IERC20Parser) parseProxyUnStaking(base protocol.IERCTransactionBase, data []byte) (*protocol.StakingCommand, error) {
	...
	for _, item := range e.Details {
		records = append(records, &protocol.StakingDetail{
			Protocol:  base.Protocol,
			Operate:   base.Operate,
			Staker:    strings.ToLower(item.Staker), // 要解除质押的人
			Pool:      pool,
			PoolSubID: poolSubID,
			Tick:      item.Tick,
			Amount:    item.Amount,
